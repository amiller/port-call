...
# Rebuilt from the ledger every run, so it can never disagree with it.
{
  ...
  tac "$ARCHIVE/ledger.tsv" | while IFS=$'\t' read -r when rig bench score stamp; do
    ...
    [ -f "$ARCHIVE/$stamp/e2e/report.html" ] && printf '<a href="%s/e2e/report.html">report</a> · ' "$stamp"
    printf '<a href="%s/bench.log">bench.log</a> · <a href="%s/e2e.log">e2e.log</a>\n' "$stamp" "$stamp"
  done
  echo "</table>"
} >"$ARCHIVE/index.html"
