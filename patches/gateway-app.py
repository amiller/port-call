...
def create_app(
    ...
    *,
    ...
    # ---- ADMIN passthrough: the single-port constraint made this necessary. ----
    # admin-api already binds 0.0.0.0:8001, but a dstack gateway exposes exactly ONE port per app
    # and a webhost pod tenant is one container — so :8001 has no hostname of its own, and
    # relaunch.sh's `docker exec … curl 127.0.0.1:8001/admin/users` has no equivalent off-box.
    # Without this, a hosted deployment can never mint a bot token and POST /bots is unreachable
    # no matter how healthy /health looks.
    #
    # NOT _forward: that resolves an x-api-key to a user via /internal/validate, and these routes
    # predate any user existing — minting the first token is the point. The guard is admin-api's
    # own X-Admin-API-Key, checked downstream, so this hands the header over and judges nothing.
    # The gateway adds no authority it did not already have; it only stops being the reason the
    # authority is unreachable.
    async def _forward_admin(method: str, path: str, request: Request) -> Response:
        key = request.headers.get("x-admin-api-key")
        if not key:
            # Fail closed and say which header, rather than letting admin-api answer a bare 401 for
            # a request that never carried a credential at all.
            return JSONResponse({"detail": "X-Admin-API-Key required"}, status_code=401)
        try:
            resp = await downstream.request(
                method,
                _admin(path),
            ...
            return JSONResponse({"detail": "admin-api timed out"}, status_code=504)
    ...
    async def admin_create_user(request: Request):
        return await _forward_admin("POST", "/admin/users", request)

    @app.post("/admin/users/{user_id}/tokens", status_code=201)
    async def admin_mint_token(user_id: int, request: Request):
        return await _forward_admin("POST", f"/admin/users/{user_id}/tokens", request)

    @app.get("/admin/users")
    async def admin_list_users(request: Request):
        return await _forward_admin("GET", "/admin/users", request)
