...
export const googleRemovalIndicators: string[] = [
  ...
  //
  // The bare '[role="alert"]' / '[role="alertdialog"]' entries were REMOVED (2026-08-11): they
  // match ANY Meet toast, and Meet raises alert-role notifications constantly — "You're
  // presenting to everyone", "Microphone muted by system", "You raised your hand". The removal
  // monitor therefore read a routine notification as a host eviction and left the call.
  // Measured: every bot that started a screen share left 20-30s after going active with
  // leave_reason="evicted", while an identical bot that never shared stayed indefinitely.
  // Scoped to actual removal wording so a banner is not mistaken for being kicked.
  '[role="alertdialog"]:has-text("removed")',
  '[role="alert"]:has-text("removed from the meeting")',
  '.error-message',
  '.connection-error',
  '.meeting-error'
];
