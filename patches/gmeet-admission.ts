...
export async function checkForGoogleAdmissionIndicators(page: Page): Promise<boolean> {
  ...
  // truly participating until it is answered — reporting admitted here yields
  // "status active, 0 transcriptions" (Vexa-ai/vexa#429), so admission stays
  // suppressed for THIS round either way; accepting simply lets the NEXT poll through.
  //
  // The accept lives HERE rather than at the escalation site in the polling loop below,
  // because that loop is not the path real joins take: on 2026-08-12 bot 71 logged this
  // very line every 2s and never once reached it, so an accept placed there was dead
  // code. Every admission flow passes through this predicate.
  if (await hasConsentPrompt(page)) {
    if (process.env.VEXA_AUTO_CONSENT === "0") {
      log(`⚠️ Gemini consent prompt visible — suppressing admission (VEXA_AUTO_CONSENT=0; waiting for a human)`);
    } else {
      log(`🧑‍⚖️ Gemini consent prompt visible — accepting it (Join now); admission resumes next poll`);
      await acceptConsentPrompt(page);
    }
    return false;
  }
...
// Accept the Gemini consent gate by clicking its "Join now".
//
// Upstream deliberately refuses to auto-click this and escalates to a human instead
// (Vexa-ai/vexa#429). In practice that strands the bot: on 2026-08-12 a human HAD already admitted
// it, yet it looped "consent prompt visible" every 2s and would have died at the 600s admission
// timeout — twice, in two different meetings. The prompt only says the call is being transcribed,
// which is precisely what a transcription bot is there for, so accepting is the intended answer.
// Set VEXA_AUTO_CONSENT=0 to restore the escalate-and-wait behaviour.
//
// The dialog sits on top of a "Got it" self-view onboarding tooltip that overlays the toolbar;
// leave it up and later toolbar acts (reactions, present) read isVisible:false, so clear it too.
export async function acceptConsentPrompt(page: Page): Promise<void> {
  await page.locator('button:has-text("Join now")').first().click({ timeout: 10_000, force: true });
  await page.waitForTimeout(1500);
  const gotIt = page.locator('button:has-text("Got it")').first();
  if (await gotIt.isVisible()) {
    await gotIt.click({ timeout: 5_000, force: true });
  }
}
...
export async function waitForGoogleMeetingAdmission(
  ...
  try {
    ...
    if (await hasConsentPrompt(page)) {
      if (process.env.VEXA_AUTO_CONSENT === "0") {
        log("🧑‍⚖️ Gemini consent prompt detected — VEXA_AUTO_CONSENT=0, escalating to needs_human_help.");
        await triggerEscalation(botConfig, "consent_required");
      } else {
        log("🧑‍⚖️ Gemini consent prompt detected — accepting it (Join now).");
        await acceptConsentPrompt(page);
      }
    }
