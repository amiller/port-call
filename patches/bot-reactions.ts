...
export function createReactionController(page: Page): ReactionController {
  return {
    async send(emoji: string): Promise<void> {
      ...
      // sufficient here.
      const pick = (e: string) => page.evaluate((e: string) => {
      ...
      // The picker STAYS OPEN after a send and REACT_BTN TOGGLES it, so clicking the toolbar
      // unconditionally shut the picker on every other call — the lookup then saw only toolbar
      // buttons and threw, giving a live alternating sent/missing/sent (observed 2026-08-12).
      // Open it only when it is not already open.
      let res = await pick(emoji);
      if (!res.ok) {
