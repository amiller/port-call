...
export function getAuthenticatedBrowserArgs(): string[] {
  return [
    ...
    '--in-process-gpu',
    // NOT --use-fake-ui-for-media-stream: it auto-answers EVERY media prompt including
    // getDisplayMedia, and it answers with the SCREEN (Create(source=screen:0:0)) — which cannot
    // start in this image, so Meet shows "Can't share your screen". This narrower flag grants
    // camera/mic without touching display capture, leaving the tab picker to be answered by
    // --auto-select-tab-capture-source-by-title. Measured 2026-08-11. (@vexa/join browser-args.ts)
