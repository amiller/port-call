(function(){ function installHud(){
    ...
    const S = { title: 'VEXA', sub: '', lines: [], speaker: '', segments: 0,
    ...
    const wrapFit = (t, font, pad = 24) => {
        ...
        for (const w of words) {
            ...
            if (ctx.measureText(nx).width > SAFE_W - pad) {
                if (cur)
                    out.push(cur);
                cur = w;
            }
    ...
    const clipFit = (t, font, pad = 24) => {
        ctx.font = font;
        let x = String(t);
        while (x.length > 1 && ctx.measureText(x).width > SAFE_W - pad)
            x = x.slice(0, -1);
        return x;
    };
    const wrap = (t, max) => {
    ...
    const AVATARS = {};
    AVATARS.rooster = (cx, cy, sc, t, state) => {
        ...
        ctx.beginPath();
        ctx.ellipse(0, 200, 150, 120, 0, 0, 7);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.ellipse(0, -30, 118, 112, 0, 0, 7);
        ctx.fill();
        // eyes — both visible, so it is looking AT you
        for (const ex of [-46, 46]) {
            ctx.fillStyle = '#fff';
            ctx.beginPath();
            ctx.ellipse(ex, -46, 30, blink ? 4 : 31, 0, 0, 7);
            ctx.fill();
            if (!blink) {
                ctx.fillStyle = '#0b0b10';
                const look = state === 'listening' ? Math.sin(t / 18) * 7 : 0;
                ctx.beginPath();
                ctx.arc(ex + look, -44, 15, 0, 7);
                ctx.fill();
                ctx.fillStyle = '#fff';
                ctx.beginPath();
                ctx.arc(ex + look + 5, -50, 5, 0, 7);
                ctx.fill();
            }
            else {
        ...
        ctx.fillStyle = '#e0343f';
        ctx.beginPath();
        ctx.ellipse(-22, 66 + beak * 0.3, 16, 26, 0, 0, 7);
        ctx.fill();
        ctx.beginPath();
        ctx.ellipse(22, 66 + beak * 0.3, 16, 26, 0, 0, 7);
        ctx.fill();
    ...
    AVATARS.hancock = (cx, cy, sc, t, state) => {
        const bob = Math.sin(t / (state === 'listening' ? 7 : 15)) * (state === 'listening' ? 9 : 4);
        const sway = Math.sin(t / 33) * 0.05;
        const bp = t % 240;
        const blink = bp < 7 || (bp > 15 && bp < 21);
        ...
        ctx.beginPath();
        ctx.ellipse(0, 210, 168, 130, 0, 0, 7);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.ellipse(-52 + flop * 0.3, -140, 26, 15, 0.2, 0, 7);
        ctx.fill();
        // face — the close crop: red fills the frame, no white cartoon head
        ctx.fillStyle = RED;
        ctx.beginPath();
        ctx.ellipse(0, -22, 132, 126, 0, 0, 7);
        ctx.fill();
        ctx.fillStyle = RED_SH; // cheek hollows
        ctx.beginPath();
        ctx.ellipse(-88, 10, 30, 52, 0.3, 0, 7);
        ctx.fill();
        ctx.beginPath();
        ctx.ellipse(88, 10, 30, 52, -0.3, 0, 7);
        ctx.fill();
        ...
        if (beak > 1) {
            ...
            ctx.moveTo(-26, 14);
            ctx.lineTo(26, 14);
            ctx.lineTo(0, 62 + beak);
            ctx.closePath();
            ctx.fill();
            ctx.fillStyle = BONE_SH;
            ctx.beginPath();
            ctx.moveTo(-26, 40 + beak * 0.5);
            ctx.lineTo(26, 40 + beak * 0.5);
            ctx.lineTo(0, 62 + beak);
            ctx.closePath();
            ctx.fill();
        }
        ...
        ctx.beginPath();
        ctx.ellipse(-30, 108 + beak * 0.45, 9, 20, 0.12, 0, 7);
        ctx.fill();
        ctx.restore();
    };
    ...
    AVATARS.tina = (cx, cy, sc, t, state) => {
        ...
        ctx.beginPath();
        ctx.arc(-113, -60, 7, 0, 7);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(113, -60, 7, 0, 7);
        ctx.fill();
        ctx.fillStyle = '#d9d5cc';
        rrf(-110, -37, 220, 147, 20); // mid + bottom rows
        ctx.fillStyle = '#a6a198';
        poly([[-110, 110], [-56, 110], [-110, 56]]);
        poly([[110, 110], [56, 110], [110, 56]]);
        ...
        ctx.save();
        ctx.translate(0, -37);
        ctx.rotate(twist); // top row twists like a cube face
        ctx.fillStyle = '#d9d5cc';
        rrf(-110, -73, 220, 76, 20);
        ctx.strokeStyle = '#8f8a80';
        ctx.beginPath();
        ctx.moveTo(-37, -68);
        ctx.lineTo(-37, -6);
        ...
        ctx.lineTo(37, -6);
        ctx.stroke();
        ctx.fillStyle = '#e03d2c';
        ctx.beginPath();
        ctx.ellipse(0, -38, 24, 18, 0, 0, 7);
        ctx.fill();
        ctx.fillStyle = '#f4938a';
        ctx.beginPath();
        ctx.ellipse(-8, -44, 7, 4, -0.5, 0, 7);
        ctx.fill();
        ctx.restore();
        ctx.fillStyle = '#17181c';
        rrf(-95, -30, 190, 60, 18); // visor
        for (const ex of [-50, 50]) {
            const look = state === 'listening' ? Math.sin(t / 16) * 6 : 0;
            ctx.fillStyle = '#37cdc9';
            ctx.beginPath();
            ctx.ellipse(ex + look, -1, 16, blink ? 3 : 20, 0, 0, 7);
            ctx.fill();
            if (!blink) {
                ctx.fillStyle = '#c8f4f2';
                ctx.beginPath();
                ctx.ellipse(ex + look - 5, -8, 4.5, 6, 0, 0, 7);
                ctx.fill();
            }
        }
        ctx.fillStyle = '#17181c';
        rrf(-28, 58, 56, mouth, 6); // mouth slot opens with amplitude
        if (mouth > 22) {
            ctx.fillStyle = '#37cdc9';
            ctx.fillRect(-20, 58 + mouth - 8, 40, 3);
        }
        ctx.restore();
    };
    // dmarz — flat-plane portrait, one big eye. Mouth opens with amplitude like the others.
    AVATARS.dmarz = (cx, cy, sc, t, state) => {
        const bob = Math.sin(t / (state === 'listening' ? 7 : 15)) * (state === 'listening' ? 9 : 4);
        const sway = Math.sin(t / 33) * 0.05;
        ...
        ctx.rotate(sway);
        ctx.scale(sc, sc);
        ctx.fillStyle = '#1d3fbd';
        ctx.fillRect(98, -170, 78, 348); // cobalt backdrop panels
        ctx.fillStyle = '#2f6de0';
        ctx.fillRect(142, -170, 22, 348);
        ctx.fillStyle = '#191921';
        poly([[-95, -142], [65, -152], [95, -100], [-105, -95]]); // hair
        ctx.fillStyle = '#1f47d8';
        ctx.beginPath();
        ctx.arc(-55, -105, 32, 0, 7);
        ctx.fill();
        ctx.fillStyle = '#e02420';
        ctx.fillRect(-25, -135, 62, 38);
        ctx.fillStyle = '#6db9e8';
        poly([[8, -135], [88, -118], [82, -45], [0, -55]]);
        ctx.fillStyle = '#2c7a3f';
        poly([[-95, -90], [-30, -100], [-45, -25]]);
        ctx.fillStyle = '#c8b48e';
        ctx.fillRect(-100, -40, 86, 86);
        const pulse = 1 + Math.sin(t / 22) * 0.045;
        ctx.fillStyle = '#e02420';
        ctx.beginPath();
        ctx.arc(-57, 3, 33 * pulse, 0, 7);
        ctx.fill();
        ctx.fillStyle = '#f5cf16';
        poly([[-95, 50], [-18, 48], [-6, 140], [-105, 135]]);
        ctx.fillStyle = '#7fc6e8';
        poly([[0, -60], [86, -50], [90, 95], [5, 118]]);
        ctx.fillStyle = '#3f9fbf';
        poly([[2, 30], [90, 40], [90, 95], [5, 118]]);
        ctx.fillStyle = '#1f47d8';
        poly([[-12, -45], [8, -50], [2, 45], [-14, 42]]);
        ctx.fillStyle = '#e6a0ac';
        poly([[88, -15], [113, -22], [116, 35], [92, 38]]);
        ctx.save();
        ctx.translate(42, -12); // the big eye
        ctx.fillStyle = '#f2f6fa';
        ctx.beginPath();
        ctx.ellipse(0, 0, 24, blink ? 3 : 20, 0, 0, 7);
        ctx.fill();
        if (!blink) {
            const look = state === 'listening' ? Math.sin(t / 18) * 5 : 0;
            ctx.fillStyle = '#2c6fd6';
            ctx.beginPath();
            ctx.arc(look, 0, 12, 0, 7);
            ctx.fill();
            ctx.fillStyle = '#0b0b10';
            ctx.beginPath();
            ctx.arc(look, 0, 6, 0, 7);
            ctx.fill();
            ctx.fillStyle = '#fff';
            ctx.beginPath();
            ctx.arc(look + 3, -4, 2.5, 0, 7);
            ctx.fill();
        }
        ...
        ctx.fillStyle = '#58b7e6';
        ctx.fillRect(-40, 135, 72, 40); // neck
        ctx.restore();
    };
    // ── the backgrounds ──────────────────────────────────────────────────────────────────────
    ...
    BACKGROUNDS.transcript = (t, state, hot) => {
        ctx.textAlign = 'center';
        ctx.fillStyle = hot ? 'rgba(124,226,139,0.9)' : 'rgba(90,90,112,0.9)';
        ctx.fillText(clipFit((S.speaker || 'listening…').toUpperCase(), 'bold 36px system-ui, sans-serif'), W / 2, 52);
        ...
        rows.forEach((r, i) => ctx.fillText(r, W / 2, 136 + i * 60));
        ctx.fillStyle = 'rgba(76,76,102,0.95)';
        ctx.font = '26px system-ui, sans-serif';
        S.lines.slice(-3, -1).reverse().forEach((l, i) => ctx.fillText(clipFit(l, '26px system-ui, sans-serif'), W / 2, 96 + rows.length * 62 + 34 + i * 30));
    };
    ...
    BACKGROUNDS.vitals = (t, state, hot) => {
        ...
        cell(700, 96, 'SINCE LAST', sinceS < 0 ? 'never' : sinceS.toFixed(1) + 's', sinceS < 0 ? red : (sinceS < 6 ? green : amber));
        cell(1010, 96, 'UPTIME', Math.round(upS) + 's', dim);
        ctx.textAlign = 'left';
        ctx.fillStyle = 'rgba(255,255,255,0.30)';
        ctx.font = '22px system-ui, sans-serif';
        ctx.fillText('SPEAKER', 70, 232);
        ctx.fillStyle = S.speaker ? '#ffffff' : dim;
    ...
    const swarm = new Map(); // word -> {n, x, y, vx, vy, seen, hue}
    let swarmSeg = -1, shiftPulse = 0, prevVocab = new Set();
    BACKGROUNDS.swarm = (t, state, hot) => {
        const amp = Math.min(S.amplitude * 2, 1);
        ...
        if (S.segments !== swarmSeg) {
            ...
            if (prevVocab.size && fresh.size) {
                let shared = 0;
                fresh.forEach((w) => { if (prevVocab.has(w))
                    shared++; });
            ...
            if (swarm.size > 80) { // bound the field, oldest go first
                const old = [...swarm.entries()].sort((a, b) => a[1].seen - b[1].seen);
                for (let i = 0; i < old.length - 80; i++)
                    swarm.delete(old[i][0]);
            }
        }
        ...
        if (shift > 0.05) { // the shift, made visible
            ...
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.arc(W / 2, H / 2, 320 * (1.4 - shift), 0, Math.PI * 2);
            ctx.stroke();
        }
    };
    BACKGROUNDS.brainrot = (t, state, hot) => {
        const amp = Math.min(S.amplitude * 2, 1);
    ...
    const draw = () => {
        ...
        if (S.sub) {
            ...
            if (age < 14) {
                ...
                lines.forEach((l, i) => ctx.fillText(l, W / 2, by + 66 + i * 34));
            }
        }
    ...
    const takeVideo = () => {
        ...
        st.getVideoTracks().forEach((t) => out.addTrack(t));
        return out;
    };
    ...
    g.__vexaCam = {
        ...
        hud(patch) {
            ...
            if (patch.line) {
                S.lines.push(patch.line);
                if (S.lines.length > 12)
                    S.lines.shift();
                S.segments++;
                S.lastAt = Date.now();
            }
            if (patch.speaker) {
                if (patch.speaker !== S.speaker)
                    S.speakerAt = S.frames; // speaker CHANGE, for the skins
                S.speaker = patch.speaker;
            }
            if (patch.speaking !== undefined) {
                S.speaking = !!patch.speaking;
                if (!patch.speaking)
                    S.amplitude = 0;
            }
            ...
            if (patch.windingUp !== undefined) {
                S.windingUp = !!patch.windingUp;
                if (patch.windingUp)
                    S.windUpStart = S.frames;
            }
            if (patch.amplitude !== undefined) {
                S.amplitude = patch.amplitude;
            }
            if (patch.title)
                S.title = patch.title;
    ...
    if (md && md.getUserMedia) {
        ...
        md.getUserMedia = async (c) => {
            if (c && c.video) {
                ...
                if (c.audio) {
                    try {
                        (await orig({ audio: c.audio })).getAudioTracks().forEach((t) => s.addTrack(t));
                    }
 ...
 installHud(); return (typeof globalThis.__vexaCam !== 'undefined') ? globalThis.__vexaCam.version : 'no __vexaCam'; })()